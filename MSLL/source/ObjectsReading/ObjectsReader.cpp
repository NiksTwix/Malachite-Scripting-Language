...
namespace MSLL
{
	namespace MD = LinkDefinitions::MSLOData;
	void ObjectsReader::HandleLOSection(LinkDefinitions::static_bpointer bytes_buffer, std::shared_ptr<LinkDefinitions::LinkingState> state, size_t section_offset, size_t section_size)
	{
		size_t offset = section_offset;

		while (offset < section_offset + section_size)
		{
			LinkDefinitions::moduleid m_id;
			memcpy(&m_id, bytes_buffer.ptr + offset, sizeof(LinkDefinitions::moduleid));
			state->linking_order.push_back(m_id);
			offset += sizeof(LinkDefinitions::moduleid);
		}

	}
	void ObjectsReader::HandleConstantsSection(LinkDefinitions::static_bpointer bytes_buffer, std::shared_ptr<LinkDefinitions::LinkingState> state, size_t section_offset, size_t section_size)
	{
		size_t offset = section_offset;
		while (offset < section_offset + section_size)
		{
	...
	void ObjectsReader::HandleSymbolSection(LinkDefinitions::static_bpointer bytes_buffer, std::shared_ptr<LinkDefinitions::LinkingState> state, size_t section_offset, size_t section_size)
	{
		size_t offset = section_offset;
		while (offset < section_offset + section_size)
		{
			...
			offset += sizeof(size_t);
			LinkDefinitions::moduleid module_;
			memcpy(&module_, bytes_buffer.ptr + offset, sizeof(LinkDefinitions::moduleid));
			offset += sizeof(LinkDefinitions::moduleid);
			LinkDefinitions::SymbolType stype = LinkDefinitions::SymbolType::Undefined;
			memcpy(&stype, bytes_buffer.ptr + offset, sizeof(LinkDefinitions::SymbolType));
			offset += sizeof(LinkDefinitions::SymbolType);


			state->symbols.push_back(LinkDefinitions::SymbolData(s_id, offset_in_code_base, module_, stype));
		}
	}
	std::shared_ptr<LinkDefinitions::LinkingState> ObjectsReader::DeserializeMO(LinkDefinitions::static_bpointer bytes_buffer)
	{
		...
		std::shared_ptr<LinkDefinitions::LinkingState> linking_state = std::make_shared<LinkDefinitions::LinkingState>();
		...
		linking_state->module_extention = std::string(module_extension);
		linking_state->module_prefix = std::string(module_prefix);

		//Sections reading

		for (size_t i = 0; i < MD::section_count; i++) 
		{
			LinkDefinitions::SectionType section_type = static_cast<LinkDefinitions::SectionType>(*(bytes_buffer.ptr + offset));
			offset += sizeof(LinkDefinitions::SectionType);
			...
			{
				std::cerr << "Invalid section data: offet + size of section is greater than size of file. Offset: " << std::hex << offset - 2 *sizeof(size_t) - sizeof(LinkDefinitions::SectionType) << "\n";
				return nullptr;
			}

			switch (section_type)
			{
			case MSLL::LinkDefinitions::LINKING_ORDER:
				HandleLOSection(bytes_buffer, linking_state, section_offset, section_size);
				break;
			case MSLL::LinkDefinitions::CONSTANTS:
				HandleConstantsSection(bytes_buffer, linking_state, section_offset, section_size);
				break;
			case MSLL::LinkDefinitions::SYMBOLS:
				HandleSymbolSection(bytes_buffer, linking_state, section_offset, section_size);
				break;
			case MSLL::LinkDefinitions::DEBUG:
				//In the future
				break;
			default:
				break;
	...
	std::shared_ptr<LinkDefinitions::CommandsPool> ObjectsReader::DeserializeCO(LinkDefinitions::static_bpointer bytes_buffer)
	{
		...
		std::shared_ptr<LinkDefinitions::CommandsPool> result = std::make_shared<LinkDefinitions::CommandsPool>();
		...
		{
			LinkDefinitions::ByteOpCode opcode;
			LinkDefinitions::CommandSource source0;
			size_t arg0;
			LinkDefinitions::CommandSource source1;
			size_t arg1;
			LinkDefinitions::CommandSource source2;
			size_t arg2;
			uint32_t flags;
	...
	LinkDefinitions::static_bpointer ObjectsReader::ReadFile(const std::string& path)
	{
