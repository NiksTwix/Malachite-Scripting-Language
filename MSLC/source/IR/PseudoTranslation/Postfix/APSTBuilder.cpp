	...
	namespace IntermediateRepresentation
	{
		namespace Pseudo
		{
			...
			TokensGroup APSTBuilder::HandleOperator(std::vector<TokensGroup>& stack, TokensGroup& _operator)
			{
				...
				{
					Dia::Logger::Get().Print(Dia::InformationMessage("APSTBuilder: Undefined operator.", Dia::MessageType::DeveloperError, Dia::SourceCode, _operator.declaring_place));
					return {};
				}
				...
				if (info.type == OperatorInfo::Type::Unary)
				{
					if (stack.empty())
					{
						Dia::Logger::Get().Print(Dia::InformationMessage("Unary operator expects operand.", Dia::MessageType::SyntaxError, Dia::SourceCode, _operator.declaring_place));
						return{};
					}
				...
				if (info.type == OperatorInfo::Type::Binary)
				{
					...
					{
						//InsertError
						Dia::Logger::Get().Print(Dia::InformationMessage("Binary operator expects two operands.", Dia::MessageType::SyntaxError, Dia::SourceCode, _operator.declaring_place));
						return{};
					}
				...
				if (info.type == OperatorInfo::Type::Declaration) 
				{
					...
					{
						//InsertError
						Dia::Logger::Get().Print(Dia::InformationMessage("Declaration's operator expects left and right values.", Dia::MessageType::SyntaxError, Dia::SourceCode, _operator.declaring_place));
						return {};
					}
					...
					{
						Dia::Logger::Get().Print(Dia::InformationMessage("Declaration of variable is impossible because identifier is invalid.", Dia::MessageType::SyntaxError, Dia::SourceCode, _operator.declaring_place));
						return {};
					}
				...
				if (info.type == OperatorInfo::Type::Assignment) //Maybe declaration they 
				{
					...
					{
						//InsertError
						Dia::Logger::Get().Print(Dia::InformationMessage("Assignment's operator expects left and right values.", Dia::MessageType::SyntaxError, Dia::SourceCode, _operator.declaring_place));
						return {};
					}
					auto right = stack.back(); stack.pop_back();
				...
				Dia::Logger::Get().Print(Dia::InformationMessage("APSTBuilder: Operator hasnt be handled.", Dia::MessageType::DeveloperError, Dia::SourceCode, _operator.declaring_place));
				return {};
			}
			TokensGroup APSTBuilder::HandleToOperator(std::vector<TokensGroup>& stack, TokensGroup& to_op)
			{
				...
				if (stack.size() < 2)
				{
					Dia::Logger::Get().Print(Dia::InformationMessage("Type casting operator expects two operands.", Dia::MessageType::SyntaxError, Dia::SourceCode, to_op.declaring_place));
					return TokensGroup();
				}
			...
			TokensGroup APSTBuilder::HandleNewOperator(std::vector<TokensGroup>& stack, TokensGroup& new_op)
			{
				...
				if (stack.empty()) {
					Dia::Logger::Get().Print(Dia::InformationMessage("Object's creation operator expects one operand.", Dia::MessageType::SyntaxError, Dia::SourceCode, new_op.declaring_place));
					return TokensGroup();
				}
				...
				for (size_t i = 0; i < root.complex.size(); i++)
				{
					...
					else 
					{
						...
						if (group.type == GroupType::DataAccess)
						{
							...
							{
								Dia::Logger::Get().Print(Dia::InformationMessage("Operator [] expects one argument.", Dia::MessageType::SyntaxError, Dia::SourceCode, group.declaring_place));
								continue;
							}
