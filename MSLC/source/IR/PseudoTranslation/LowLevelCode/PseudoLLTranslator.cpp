	...
	namespace IntermediateRepresentation
	{
		namespace Pseudo
		{
			...
			void LLTranslator::Translate(PseudoTranslationState& state,const AST::ASTNode& node)
			{
				...
				state.pseudo_code.Pushback(PseudoOperation(PseudoOpCode::PushLLOpers, chunks_id,node.declaring_place));
				...
				for (const AST::ASTNode& cnode : node.children)
				{
					...
					{
						Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("In " + std::string(Keywords::w_op_code) + " section are allowed only simple instructions.", Diagnostics::MessageType::SyntaxError, Diagnostics::SourceType::SourceCode, cnode.declaring_place));
						continue;
					}
					...
					for (const Token& t : cnode.tokens) 
					{
						if (t.value.strVal == ":") 
						{
							if (arguments_) 
							{
								Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("In " + std::string(Keywords::w_op_code) + " section repeated using ':' for arguments separating is forbidden.", Diagnostics::MessageType::SyntaxError, Diagnostics::SourceType::SourceCode, cnode.declaring_place));
								checking = false;
							}
							arguments_ = true;
							continue;
						...
						if (t.value.strVal == ",") 
						{
							if (!current.valid) 
							{
								Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("In " + std::string(Keywords::w_op_code) + " section operation contains invalid argument.", Diagnostics::MessageType::SyntaxError, Diagnostics::SourceType::SourceCode, cnode.declaring_place));
								checking = false;
								continue;
							}
