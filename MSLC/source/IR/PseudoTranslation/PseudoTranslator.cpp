		...
		namespace Pseudo
		{
			void PseudoTranslator::AnalyzeRecursive(AST::ASTNode& node, PseudoTranslationState& pts)
			{
				if (node.tokens.size() == 1 && node.tokens.front().type == Tokenization::TokenType::COMPILATION_LABEL) 
				{
					...
					{
						pts.pseudo_code.Pushback(PseudoOperation(PseudoOpCode::PushFrame,0,0,0,node.tokens.front().declaring_place));
					...
					{
						pts.pseudo_code.Pushback(PseudoOperation(PseudoOpCode::PopFrame, 0, 0, 0, node.tokens.front().declaring_place));
