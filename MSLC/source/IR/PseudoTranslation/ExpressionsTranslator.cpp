		...
		namespace Pseudo
		{
			...
			void ExpressionsTranslator::AnalyzeAPST(TokensGroup& node, PseudoTranslationState& pts)
			{
				...
				case GroupType::DataAccess:
				{
					...
					pts.pseudo_code.Pushback(PseudoOperation(PseudoOpCode::GetByArgument, 0, 0, 0, node.declaring_place, PseudoOperationFlags::None));
				}
					break;
				...
				case GroupType::Operation:
				{
					...
					if (operator_info.type == OperatorInfo::Type::Unary)
					{
						...
						pts.pseudo_code.Pushback(PseudoOperation(operator_info.op_code,0,0,0, node.declaring_place,PseudoOperationFlags::Unary));
					}
					else if (operator_info.type == OperatorInfo::Type::Binary)
					{
						...
						pts.pseudo_code.Pushback(PseudoOperation(operator_info.op_code, 0, 0, 0, node.declaring_place, PseudoOperationFlags::Binary));
					}
					else if (operator_info.type == OperatorInfo::Type::Assignment) 
					{
						...
						{
							pts.pseudo_code.Pushback(PseudoOperation(PseudoOpCode::Use, pts.pseudo_code.Back().arg_0, 0, 0,node.declaring_place));
						}
						...
						pts.pseudo_code.Pushback(PseudoOperation(PseudoOpCode::Assign, (size_t)operator_info.op_code, 0, 0, node.declaring_place, PseudoOperationFlags::Binary));
					}
				}
					break;
				case GroupType::Declaration://[variable]:[modifiers][type's identifier]
				{
					...
					if (symbol != nullptr) 
					{
						Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Variable \"" + identifier.simple.value.strVal +  "\" has already declared.", Diagnostics::MessageType::SyntaxError, Diagnostics::SourceCode, identifier.declaring_place));
						return;
					}
					...
					pts.pseudo_code.Pushback(PseudoOperation(PseudoOpCode::DeclareVariable, symbol->description_id, node.declaring_place));
				}
					break;
			...
			void ExpressionsTranslator::HandleSimple(TokensGroup& node, PseudoTranslationState& pts)
			{
				...
				case GroupType::Simple:
					if (node.simple.type == TokenType::IDENTIFIER) {
						...
						if (!symbol) 
						{
							Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Undefined identifier \"" + node.simple.value.ToString() + "\".", Diagnostics::SyntaxError, Diagnostics::SourceCode, node.declaring_place));
							return;
						}
						pts.pseudo_code.Pushback(PseudoOperation(PseudoOpCode::Use, symbol->description_id,0,0, node.declaring_place));
					}
					...
					{
						Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Undefined simple tokens group \"" + node.simple.value.ToString() + "\".", Diagnostics::DeveloperError, Diagnostics::SourceCode, node.declaring_place));
					}
					break;
				case GroupType::ArrayLiteral:
					...
					pts.pseudo_code.Pushback(PseudoOperation(PseudoOpCode::CreateArray, node.complex.size(), 0, 0, node.declaring_place));
					break;
				case GroupType::QualifiedName:
				{
					...
					for (size_t i = 0; i < node.complex.size(); i++) 
					{
						...
						{
							Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Invalid identifier \"" + identifier.simple.value.ToString() + "\".", Diagnostics::SyntaxError, Diagnostics::SourceCode, node.declaring_place));
							return;
						}
						...
						else 
						{
							if (current->type == CompilationInfo::SymbolType::Namespace)
							{
								...
								if (!frame->lsl.Has(str_val)) {
									Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Undefined identifier \"" + str_val + "\".", Diagnostics::SyntaxError, Diagnostics::SourceCode, node.declaring_place));
									return;
								}
							...
							{
								pts.pseudo_code.Pushback(PseudoOperation(PseudoOpCode::Use, current->description_id, 0, 0, node.declaring_place));
								return;
							}
						}
						if (current == nullptr) 
			...
			CompilationInfo::Values::ValueInfo  ExpressionsTranslator::GetValueInfoFromType(TokensGroup& node, PseudoTranslationState& pts)
			{
				...
				{ 
					...
					if (added_one_use)
					{
						...
						{
							Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Undefined type identifier \"" + node.simple.value.ToString() + "\".", Diagnostics::SyntaxError, Diagnostics::SourceCode, node.declaring_place));
							return {};
						}
					...
					{
						Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Invalid type identifier.", Diagnostics::SyntaxError, Diagnostics::SourceCode, node.declaring_place));
						return {};
					}
					return info;
				}
				...
				for (size_t i = 0; i < node.complex.size() - 1; i++) // checks identificators
				{
					...
					{
						Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Type's identifier must be one and after modifiers.", Diagnostics::SyntaxError, Diagnostics::SourceCode, node.declaring_place));
						return {};
					}
				}
				...
				if (added_one_use)
				{
					if (!pts.cs_observer->GetGST().HasType(pts.pseudo_code.Back().arg_0)) 
					{
						Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Undefined type identifier \"" + identifier.simple.value.ToString() + "\".", Diagnostics::SyntaxError, Diagnostics::SourceCode, node.declaring_place));
						return {};
					}
					info.type_id = pts.pseudo_code.Back().arg_0;	//type_description
					pts.pseudo_code.Popback();
				}
				else Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Invalid type identifier.", Diagnostics::SyntaxError, Diagnostics::SourceCode, node.declaring_place));
				
				if ((info.flags & ValueFlags::Pointer) && (info.flags & ValueFlags::Reference))
				{
					Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("References cannot be combined with pointer modifier.", Diagnostics::SyntaxError, Diagnostics::SourceCode, node.declaring_place));
					return {};
				}
