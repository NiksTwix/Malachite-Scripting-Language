	...
	namespace IntermediateRepresentation
	{
		namespace Byte
		{
			...
			void CommandsHandler::HandleCommand(std::shared_ptr<ByteTranslationState> b_state)
			{
				...
				b_state->declaring_place = operation.declaring_place;
				...
				if (operation.op_code == Pseudo::PseudoOpCode::PopFrame)
				{
					if (!b_state->PopFrame())
					{
						Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Extra visible's scope deleting. Operation line: %s.", Diagnostics::DeveloperError, Diagnostics::SourceCode, b_state->GetDeclaringPlace()));
					}
					return;
				}
