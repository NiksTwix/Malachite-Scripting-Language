			...
			namespace BCH
			{
				void ArithmeticLogicHandler::Handle(Pseudo::POperationArray& p_array, std::shared_ptr<ByteTranslationState> b_state)
				{
					...
					auto main_ari_handler = [&]() -> void
						{
							...
							{
								Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Binary operation gets less than two operands.", Diagnostics::MessageType::DeveloperError, Diagnostics::SourceType::SourceCode, b_state->GetDeclaringPlace()));
								return;
							}
					...
					auto main_logic_handler = [&]() -> void
						{
							...
							PushCommand(b_state,
								ByteCommand(GetLogicCommand(operation.op_code, PrimitiveAnalogs::UInt), CommandArgument(left.data, CommandSource::Register),
									CommandArgument(left.data, CommandSource::Register), CommandArgument(right.data, CommandSource::Register)), b_state->GetDeclaringPlace().place);
					...
					auto main_bit_logic_handler = [&]() -> void
						{
							...
							if (left.dynamic_primitive_type == PrimitiveAnalogs::Real || right.dynamic_primitive_type == PrimitiveAnalogs::Real) {
								Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Bit operation requires integer types.", Diagnostics::MessageType::LogicError, Diagnostics::SourceType::SourceCode, b_state->GetDeclaringPlace()));
								return;
							}
					...
					case Pseudo::PseudoOpCode::Mod:
					{
						...
						if (left.dynamic_primitive_type == PrimitiveAnalogs::Real || right.dynamic_primitive_type == PrimitiveAnalogs::Real) {
							Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Mod operation requires integer types.", Diagnostics::MessageType::LogicError, Diagnostics::SourceType::SourceCode, b_state->GetDeclaringPlace()));
							return;
						}
						size_t converted_reg;
						PrimitiveAnalogs common_type;
					...
					case Pseudo::PseudoOpCode::Negative:
					{
						...
						{
							Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Negative is unary operation, but gets zero arguments.", Diagnostics::MessageType::LogicError, Diagnostics::SourceType::SourceCode, b_state->GetDeclaringPlace()));
							break;
						}
						...
						if (vf_left.dynamic_primitive_type == PrimitiveAnalogs::UInt) {
							Diagnostics::Logger::Get().Print(Diagnostics::InformationMessage("Negative operation requires signed types (int, real).", Diagnostics::MessageType::TypeError, Diagnostics::SourceType::SourceCode, b_state->GetDeclaringPlace()));
							break;
						}
						...
						PushCommand(b_state, ByteCommand(
							...
							CommandArgument(vf_left.data, CommandSource::Register)), b_state->GetDeclaringPlace().place);
